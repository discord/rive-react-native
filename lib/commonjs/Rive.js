...
const RiveContainer = /*#__PURE__*/_react.default.forwardRef(({
  ...
  (0, _react.useEffect)(() => {
    if (!onReady) return () => {};
    const viewTag = (0, _reactNative.findNodeHandle)(riveRef.current);
    if (!viewTag) return () => {};
    const subscription = nativeEventEmitter.addListener(`RiveReactNativeLoaded:${viewTag}`, () => {
      onReady();
    });
    return () => {
      subscription.remove();
  ...
  const setImage = (0, _react.useCallback)((path, imageUrl) => {
    console.log('[RiveReactNative JS] setImage called:', {
      path,
      imageUrl
    });
    _reactNative.UIManager.dispatchViewManagerCommand((0, _reactNative.findNodeHandle)(riveRef.current), _types.ViewManagerMethod.setImagePropertyValue, [path, imageUrl]);
  }, []);
  const setBase64Image = (0, _react.useCallback)((path, base64String) => {
    _reactNative.UIManager.dispatchViewManagerCommand((0, _reactNative.findNodeHandle)(riveRef.current), _types.ViewManagerMethod.setBase64ImagePropertyValue, [path, base64String]);
  }, []);
  ...
  (0, _react.useImperativeHandle)(ref, () => ({
    ...
    viewTag
  }), [play, pause, stop, reset, setInputState, getBooleanState, getBooleanStateAtPath, getNumberState, getNumberStateAtPath, setInputStateAtPath, fireState, fireStateAtPath, touchBegan, touchEnded, setTextRunValue, setTextRunValueAtPath, setBoolean, setString, setNumber, setColor, setEnum, setImage, setBase64Image, setArtboard, trigger, internalNativeEmitter, viewTag]);
  ...
  }, children), /*#__PURE__*/_react.default.createElement(RiveViewManager, {
    ref: riveRef,
