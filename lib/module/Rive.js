...
import { requireNativeComponent, UIManager, findNodeHandle, StyleSheet, View, NativeModules, NativeEventEmitter, Platform } from 'react-native';
...
const RiveContainer = /*#__PURE__*/React.forwardRef(({
  ...
  const setImage = useCallback((path, imageUrl) => {
    console.log('[RiveReactNative JS] setImage called:', {
      path,
      imageUrl
    });
    UIManager.dispatchViewManagerCommand(findNodeHandle(riveRef.current), ViewManagerMethod.setImagePropertyValue, [path, imageUrl]);
  }, []);
  const setBase64Image = useCallback((path, base64String) => {
    UIManager.dispatchViewManagerCommand(findNodeHandle(riveRef.current), ViewManagerMethod.setBase64ImagePropertyValue, [path, base64String]);
  }, []);
  ...
  }, children), /*#__PURE__*/React.createElement(RiveViewManager, {
    ref: riveRef,
